...
export const saveImagesLocally = async (image: LocalImages) => {  // kuvan paikallista tallennusta varten
    try {
        const existingImages = await AsyncStorage.getItem(STORAGE_KEY) // haetaan ensin vanhat kuvat jos niitä on jo olemassa
        ...
        await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(images)) //tallennetaan päivitetty taulukko local storageen
    } catch (error) {
...
export const getLocalImages = async (): Promise<LocalImages[]> => { // Haetaan tarvittaessa paikallisesti tallennetut kuvat
    try {
        const storedImages = await AsyncStorage.getItem(STORAGE_KEY) // haetaan local storageen tallennetut kuvat
...
export const deleteLocalImages = async () => { // kuvien poisto
    try {
        await AsyncStorage.removeItem(STORAGE_KEY) // poistetaan kaikki kuvat local storagesta
    } catch (error) {
